...
const runtimeDir = resolve(process.env.LENSMAP_RUNTIME_DIR ?? resolve(root, ".runtime"));
...
const dataDir = resolve(process.env.LENSMAP_DATA_DIR ?? resolve(root, "apps/server/data"));
const migrationsDir = resolve(root, "apps/server/drizzle");
const host = process.env.LENSMAP_HOST ?? "127.0.0.1";
const port = Number.parseInt(process.env.LENSMAP_PORT ?? "4317", 10);
...
switch (command) {
  ...
  case "status":
    ...
    break;
  default:
    console.error("Usage: node scripts/lensmap-server.mjs <start|stop|restart|status>");
    process.exitCode = 2;
}
...
async function start() {
  ...
  if (currentPid && isRunning(currentPid)) {
    console.log(`Lensmap Server is already running (pid ${currentPid}).`);
    await status();
    return;
  }

  if (!existsSync(serverEntry)) {
    throw new Error("Production server build not found. Run `npm run build -w @lensmap/server` first.");
  }
  ...
  if (!ready) {
    ...
    throw new Error(`Lensmap Server failed to start.\n${log}`);
  }
  console.log(`Lensmap Server started (pid ${child.pid}) at ${serverBase}.`);
  await status();
}
...
async function stop() {
  ...
  if (!pid || !isRunning(pid)) {
    ...
    console.log("Lensmap Server is not running.");
    return;
  }
  ...
  console.log("Lensmap Server stopped.");
}
