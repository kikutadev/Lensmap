...
const projectRoot = process.env.LENSMAP_PROJECT_ROOT
  ? resolve(process.env.LENSMAP_PROJECT_ROOT)
  : resolve(scriptDir, "..");
const controllerPath = resolve(projectRoot, "scripts/lensmap-server.mjs");
const runtimeDir = resolve(process.env.LENSMAP_RUNTIME_DIR ?? resolve(projectRoot, ".runtime"));
...
try {
  ...
  console.error("Lensmap Native Host:", error);
  writeNativeMessage({
    ok: false,
...
async function handleRequest(request) {
  ...
  if (command === "ensure-server") {
    ...
    if (!securedHealth) {
      throw new Error("Lensmap Server startup command completed, but health check still fails.");
    }
    if (!securedHealth.capabilityRequired) {
      throw new Error("Lensmap Server is running without local capability protection.");
    }

    const capabilityToken = readCapabilityToken();
    if (!capabilityToken) {
      throw new Error("Lensmap Server capability token is unavailable.");
    }
...
async function runServerController(command) {
  try {
    ...
    throw new Error(`Lensmap Serverの${command}に失敗しました: ${detail}`);
  }
}
