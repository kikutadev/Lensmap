import type { MapBlock, GroundingKind, GroundingStatus } from "@lensmap/shared";
import { visualizationSchema } from "@lensmap/visualization";
...
export interface ParsedMapBlock extends Omit<MapBlock, "id" | "order"> {
...
/**
 * Split assistant Markdown into durable Map blocks without executing model output.
 * Mermaid, validated Visualization DSL, and GFM tables receive explicit block kinds.
 */
export function parseMarkdownMapBlocks(
  markdown: string,
  sources: MapSourceLabel[],
): ParsedMapBlock[] {
...
interface RawBlock {
  kind: "narrative" | "table" | "diagram" | "chart";
  text: string;
  content: unknown;
...
function splitMarkdown(markdown: string): RawBlock[] {
  ...
  while (index < lines.length) {
    ...
    if (fence) {
      ...
      } else if (language === "visualization" || language === "lensmap-viz") {
    ...
    const text = collected.join("\n");
    blocks.push({ kind: "narrative", text, content: { markdown: text }, declaredSourceLabels: [] });
  }

  return blocks.length > 0
    ? blocks
    : [{ kind: "narrative", text: markdown, content: { markdown }, declaredSourceLabels: [] }];
}
